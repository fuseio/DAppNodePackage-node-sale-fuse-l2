...
if [ -z "${NETWORK}" ]; then
    ...
    exit 1
elif [ -z "${AVAIL_EVM_ADDRESS}" ]; then
    echo $(date -u +"%Y-%m-%dT%H:%M:%S.%6NZ") ERROR docker-entrypoint.sh: AVAIL_EVM_ADDRESS is unset or set to the empty string.
    exit 1
else
    ...
    export REWARD_COLLECTOR_ADDRESS=${AVAIL_REWARD_COLLECTOR_ADDRESS:="0x"}
...
avail_evm_address = "$AVAIL_EVM_ADDRESS"
